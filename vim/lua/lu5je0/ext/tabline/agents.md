...
- **颜色动态推导**：`config.apply_highlights()` 从当前 colorscheme 的 `Normal`、`Comment`、`String`、`TabLineSel`、`DiagnosticError`、`WinSeparator` 读取颜色，经 `shade()` 计算后设置所有 `BufferLine*` 高亮组。`ColorScheme` autocmd 重新应用。
...
- **懒加载**：`ext-config.lua` 通过 `lazy_load` 注册，`UIEnter` 事件触发 setup。setup 内同步执行 `apply_highlights` + `autocmds` + 设置 tabline；`commands` 和 `keymaps` 延迟到 `vim.schedule`。
...
- 新增/修改高亮组：在 `config.apply_highlights()` 的 `groups` 表内操作，不要散落到其他文件。
