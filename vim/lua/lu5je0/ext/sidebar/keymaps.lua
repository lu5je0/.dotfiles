local state = require('lu5je0.ext.sidebar.state')
local config = require('lu5je0.ext.sidebar.config')

local M = {}

local _dispatch = {}
local _dispatch_initialized = false

local function buf_set(mode, lhs, rhs, opts)
  opts = opts or {}
  opts.buffer = state.buf
  opts.noremap = true
  opts.silent = true
  opts.nowait = true
  vim.keymap.set(mode, lhs, rhs, opts)
end

function M.apply_shared()
  if not state:is_buf_valid() then
    return
  end

  local tabs = require('lu5je0.ext.sidebar.tabs')
  local window = require('lu5je0.ext.sidebar.window')

  buf_set('n', '<right>', tabs.next_tab)
  buf_set('n', '<left>', tabs.prev_tab)
  buf_set('n', '<leader>1', function() tabs.switch_to(1) end)
  buf_set('n', '<leader>2', function() tabs.switch_to(2) end)
  buf_set('n', '<leader>3', function() tabs.switch_to(3) end)
  buf_set('n', '<leader>4', function() tabs.switch_to(4) end)
  buf_set('n', 'q', window.close)
  buf_set('n', 'Z', window.toggle_width)
  buf_set('n', '<leader>tn', function() end)
  buf_set('n', '<leader>ws', function() end)

  buf_set('n', 'zM', function()
    local source = tabs.get_active_source()
    if source and source.collapse_all then
      source.collapse_all()
    end
  end)

  buf_set('n', 'zR', function()
    local source = tabs.get_active_source()
    if source and source.expand_all then
      source.expand_all()
    end
  end)

  local preview_mod = require('lu5je0.ext.sidebar.actions.preview')
  local file_ops = require('lu5je0.ext.sidebar.actions.file_ops')
  buf_set('n', '<esc>', function()
    if preview_mod.is_active() then
      preview_mod.stop()
    end
    if state.files._clipboard then
      state.files._clipboard = nil
      file_ops.apply_clipboard_mark()
    end
  end)

  local help = require('lu5je0.ext.git.common.help')
  local shared_keymaps = {
    { '<left>/<right>', 'Switch tab' },
    { '<leader>1/2/3/4', 'Jump to tab' },
    { 'q', 'Close sidebar' },
    { 'Z', 'Toggle width' },
    { 'zM', 'Collapse all' },
    { 'zR', 'Expand all' },
    { '<esc>', 'Close preview / Clear clipboard' },
    { '?', 'Show help' },
  }
  buf_set('n', '?', function()
    local tab = config.tabs[state.active_tab_idx]
    local ok, source = pcall(require, 'lu5je0.ext.sidebar.sources.' .. (tab and tab.id or ''))
    local lines = { 'Shared', '' }
    for _, km in ipairs(shared_keymaps) do
      lines[#lines + 1] = string.format('  %-14s  %s', km[1], km[2])
    end
    if ok and source.keymaps then
      lines[#lines + 1] = ''
      lines[#lines + 1] = tab.label
      lines[#lines + 1] = ''
      local seen = {}
      for _, km in ipairs(source.keymaps()) do
        local desc = km.desc or ''
        if not seen[desc] then
          seen[desc] = true
          lines[#lines + 1] = string.format('  %-14s  %s', km[1], desc)
        end
      end
    end
    help.show_help('Help', lines)
  end)

  for _, lhs in ipairs(config.passthrough_mappings or {}) do
    buf_set('n', lhs, function()
      local target = window.get_target_win()
      if target then
        vim.api.nvim_set_current_win(target)
        vim.api.nvim_feedkeys(vim.api.nvim_replace_termcodes(lhs, true, false, true), 'm', false)
      end
    end)
  end

  buf_set('n', 'j', function()
    local keys = require('lu5je0.core.keys')
    keys.feedkey('j', 'n')
  end)
  buf_set('n', 'k', function()
    local keys = require('lu5je0.core.keys')
    keys.feedkey('k', 'n')
  end)
end

local function build_dispatch()
  if _dispatch_initialized then return end
  _dispatch_initialized = true

  for i, tab in ipairs(config.tabs) do
    local ok, source = pcall(require, 'lu5je0.ext.sidebar.sources.' .. tab.id)
    if ok and source.keymaps then
      for _, km in ipairs(source.keymaps()) do
        local mode = km.mode or 'n'
        local key = mode .. ':' .. km[1]
        if not _dispatch[key] then
          _dispatch[key] = { mode = mode, lhs = km[1], handlers = {} }
        end
        _dispatch[key].handlers[i] = km[2]
      end
    end
  end
end

function M.apply_for_tab(_idx)
  if not state:is_buf_valid() then return end
  if vim.b[state.buf]._sidebar_keymaps_bound then return end
  vim.b[state.buf]._sidebar_keymaps_bound = true

  build_dispatch()

  for _, entry in pairs(_dispatch) do
    buf_set(entry.mode, entry.lhs, function()
      local h = entry.handlers[state.active_tab_idx]
      if h then h() end
    end)
  end
end

return M
