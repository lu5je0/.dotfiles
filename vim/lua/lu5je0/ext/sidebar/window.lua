...
local win_hl_ns = vim.api.nvim_create_namespace('sidebar_win_hl')
...
do
  ...
  statusline.register_component('sidebar_label', {
    function()
      return '%#StatusLineGrey#' .. config.filetype:upper()
    end,
  })

  statusline.register_component('sidebar_clipboard', {
    function()
  ...
  statusline.register_component('sidebar_gps', {
    function(args)
  ...
  statusline.append_config({
    ...
    left = { { name = 'sidebar_label' }, { name = 'sidebar_gps' } },
    right = { { name = 'sidebar_clipboard' }, 'tabpages' },
  })
end
...
function M.setup_guicursor()
  ...
  local group = vim.api.nvim_create_augroup('sidebar-guicursor', { clear = true })
