...
r.run('untracked file produces TreeSidebarGitNew', function()
...
r.run('staged add produces TreeSidebarGitStaged', function()
  ...
  r.assert_eq(map['staged.txt'].hl, 'TreeSidebarGitStaged')
end)

r.run('unstaged modification produces TreeSidebarGitDirty', function()
  local map = files._build_git_status_map(' M dirty.txt\0')
  r.assert_eq(map['dirty.txt'].hl, 'TreeSidebarGitDirty')
end)
...
r.run('rename consumes the next NUL entry as old path', function()
  ...
  r.assert_eq(map['new.txt'].hl, 'TreeSidebarGitDirty')
  ...
  r.assert_eq(map['other.txt'].hl, 'TreeSidebarGitNew')
...
r.run('directory aggregation picks highest priority among children', function()
  ...
  r.assert_eq(map['pkg/'].hl, 'TreeSidebarGitDirty')
end)
...
r.run('directory aggregation excludes ignored files', function()
  ...
  r.assert_eq(map['pkg/'].hl, 'TreeSidebarGitStaged')
end)
...
r.run('nested directories all receive the aggregated status', function()
  ...
  r.assert_eq(map['a/'].hl, 'TreeSidebarGitDirty')
  r.assert_eq(map['a/b/'].hl, 'TreeSidebarGitDirty')
  r.assert_eq(map['a/b/c/'].hl, 'TreeSidebarGitDirty')
end)
...
r.run('!! ignored', function()
  ...
  r.assert_eq(hl, 'TreeSidebarGitIgnored')
end)

r.run('?? untracked', function()
  local _, hl = files._git_status_to_glyph('??')
  r.assert_eq(hl, 'TreeSidebarGitNew')
end)
...
r.run('D in either column => deleted/dirty', function()
  ...
  r.assert_eq(hl_x, 'TreeSidebarGitDirty')
  r.assert_eq(hl_y, 'TreeSidebarGitDirty')
end)

r.run('R rename => dirty', function()
  local _, hl = files._git_status_to_glyph('R ')
  r.assert_eq(hl, 'TreeSidebarGitDirty')
end)

r.run('A add => staged', function()
  local _, hl = files._git_status_to_glyph('A ')
  r.assert_eq(hl, 'TreeSidebarGitStaged')
end)
...
r.run('file_suffix produces a virt_text entry', function()
  ...
  r.assert_eq(virt_texts[1].virt_text, { { 'M', 'TreeSidebarGitDirty' } })
end)
